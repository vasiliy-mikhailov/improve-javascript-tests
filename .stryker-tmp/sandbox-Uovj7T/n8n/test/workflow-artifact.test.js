...
// ── no real logic may live only in the workflow ──────────────────────────────
// A Code node's body is inlined by source from n8n/nodes/*.js, which is what makes it
// testable at all: the artifact is generated, so anything written straight into it has
// no unit test and no way to get one. The rule is a size one — an expression that
// picks two fields out of a payload is fine where it stands; ten-plus lines of
// JavaScript is a function, and functions belong in a file.
const NODE_SRC = readdirSync(new URL('../nodes', import.meta.url))
  .filter((f) => f.endsWith('.js'))
  .map((f) => readFileSync(new URL(`../nodes/${f}`, import.meta.url), 'utf8'))
  .join('\n');

test('every function in a Code node comes from a file that tests can reach', () => {
  const code = wf.nodes.filter((n) => n.type === 'n8n-nodes-base.code');
  assert.ok(code.length >= 4, 'expected the generated Code nodes');
  for (const n of code) {
    const declared = [...String(n.parameters.jsCode).matchAll(/function\s+(\w+)\s*\(/g)].map((m) => m[1]);
    assert.ok(declared.length, `${n.name}: a Code node with no function is hand-written logic`);
    for (const fn of declared) {
      assert.ok(new RegExp(`function ${fn}\\s*\\(`).test(NODE_SRC),
        `${n.name}: ${fn}() exists only in the generated artifact — move it to n8n/nodes/ so it can be tested`);
    }
  }
});

test('an inline expression stays small enough not to be a function', () => {
  const LIMIT = 10;
  for (const n of wf.nodes) {
    for (const [key, val] of Object.entries(n.parameters || {})) {
      if (key === 'jsCode' || typeof val !== 'string' || !val.startsWith('=')) continue;
      const lines = val.split('\n').filter((l) => l.trim()).length;
      assert.ok(lines < LIMIT,
        `${n.name}.${key}: ${lines} lines of JavaScript inline — that is a function, put it in n8n/nodes/`);
    }
  }
});
