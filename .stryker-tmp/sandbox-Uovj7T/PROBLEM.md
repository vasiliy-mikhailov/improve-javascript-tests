...
| 16 | 2026-07-28 | **One take per file.** The mutant phase is a single sweep and there is one round: no second sweep, no single-target fallback, no reasoning retry, no `Another Round?`. Each layer was measured before it was removed — 3.0 kills per call against the sweep's 6.0, 25.3% of wall clock for 8% of kills, +0.00 MAC on five second rounds out of five. 69 nodes → 57. `MAX_ROUNDS_PER_FILE` is gone with the gate that read it. |
